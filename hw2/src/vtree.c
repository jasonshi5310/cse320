...
void getcwd(char* cwd, int i);
void getwd(char* cwd);
void chdir(char* subdir);
int	chk_4_dir(char* path);
void get_data(char* path, int v);
void free(void* p);
int	is_directory(char* path);
int h_enter(int dev,int ino);
...
int	user_file_list_supplied = 0;
	...
	int longind = 0;
	// option = getopt_long(argc, argv, "dfh:iostqvV", lops, &longind);
    /* Pick up options from command line */

#ifdef LINUX
    while ((option = getopt_long(argc, argv, "dfh:iostqvVl", lops, &longind)) != EOF) {
#else
	...
	// while ((option = getopt(argc, argv, "dfh:iostqvV")) != EOF) {
		...
		if (err) {
			...
			#ifdef LSTAT
			fprintf(stderr,"	-l	no follow symlinks\n");
			#endif
			#ifdef MEMORY_BASED
	...
	if (version > 0 ) {
		...
		if (version>1) {
			...
			#endif
			#ifdef LSTAT
			if (!sw_follow_links) printf("No follow symlinks\n");
			#endif
		}
	}
