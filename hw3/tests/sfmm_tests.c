...
// Need to write another 5 tests

Test(sf_memsuite_student, realloc_smaller_block_free_block_check_pos, .init = sf_mem_init, .fini = sf_mem_fini) {
	void *x = sf_malloc(sizeof(double) * 8);
	void *y = sf_realloc(x, sizeof(int));
...
Test(sf_memsuite_student, sf_memalign_invalid_align, .init=sf_mem_init, .fini=sf_mem_fini)
{
	sf_errno = 0;
	void* x = sf_memalign(10, 8);
	cr_assert_eq(sf_errno, EINVAL, "sf_errno is not set to EINVAL!");
	sf_errno = 0;
	void* y = sf_memalign(20, 65);
	cr_assert_eq(sf_errno, EINVAL, "sf_errno is not set to EINVAL!");
	sf_errno = 0;
	void* zz = sf_memalign(0, 64);
	cr_assert_eq(sf_errno, 0, "sf_errno is changed!");
	void* z = sf_memalign(20,64);

	cr_assert_null(x, "x is not null!");
	cr_assert_null(y, "y is not null!");
	cr_assert_null(zz, "zz is not null!");
	cr_assert_not_null(z, "z is null!");
}

Test(sf_memsuite_student, sf_memalign_aligned_free_two, .init=sf_mem_init, .fini=sf_mem_fini)
{
	void* z = sf_memalign(20,64);
	cr_assert_eq(((size_t)z%64),0, "z is not aligned!");

	assert_free_block_count(0,1);
	assert_free_list_size(9,1);
}

Test(sf_memsuite_student, sf_memalign_aligned_free_both_ends, .init=sf_mem_init, .fini=sf_mem_fini)
{
	void* z = sf_memalign(20,1024);
	cr_assert_eq(((size_t)z%1024),0, "z is not aligned!");
	assert_free_block_count(0,2);
	assert_free_list_size(4,1);
	assert_free_block_count(512,1);
}

Test(sf_memsuite_student, free_in_random_order, .init = sf_mem_init, .fini = sf_mem_fini) {
	sf_errno = 0;
	/*void *x = */ sf_malloc(8);
	void *y = sf_malloc(200);
	void *z = sf_malloc(1);
...
Test(sf_memsuite_student, merge_with_prev, .init = sf_mem_init, .fini = sf_mem_fini) {
	sf_errno = 0;
	void *x =  sf_malloc(8);
	void *y = sf_malloc(200);
	/*void *z =*/ sf_malloc(1);
