...
SYMBOL *new_rule(int v) {
    ...
    if (v < 256)
        return NULL;
    SYMBOL* nr = new_symbol(v, symbol_storage);
    SYMBOL** p = rule_map;
    p = p + v;
    *(p) = nr;
    nr -> next = nr;
...
void delete_rule(SYMBOL *rule) {
    // To be implemented.
    SYMBOL* pre = rule -> prevr;
    pre -> nextr = rule -> nextr;
    rule -> nextr -> prevr = pre;
    if (rule == main_rule)
    {
        if ((main_rule -> nextr) == main_rule)
            main_rule = NULL;
        else main_rule = rule -> nextr;
    }
    if ((rule -> refcnt) == 0)
    {
        recycle_symbol(rule);
...
void unref_rule(SYMBOL *rule) {
    ...
    if (rule != NULL)
    {
        ...
        if ((rule -> refcnt) < 0)
        {
            fprintf(stderr, "Reference count is negative");
            abort();
        }
    }
}
