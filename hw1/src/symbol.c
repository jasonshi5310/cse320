...
SYMBOL *new_symbol(int value, SYMBOL *rule) {
    ...
    {
        fprintf(stderr, "Symbol storage is exhausted!");
        abort();
    }
    if (recycled_symbols != NULL)
    {
        s = recycled_symbols;
        s -> value = value;
        if (rule!=NULL) ref_rule(rule);
        s -> rule = rule;
        recycled_symbols = recycled_symbols -> prev;
        if (recycled_symbols != NULL)
...
void recycle_symbol(SYMBOL *s) {
   // To be implemented.
    ...
    else
    {
        ...
        s -> rule = NULL;
        s -> refcnt = 0;
        recycled_symbols -> next = s;
        s -> prev = recycled_symbols;
        recycled_symbols = s;
    }
}
